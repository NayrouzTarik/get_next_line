...
/*   Updated: 2024/03/29 02:15:23 by ntarik           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (!buffer)
		return (NULL);
	bytes_reading = 1;
	while (!ft_strchr(cup[fd], '\n') && bytes_reading != 0)
	{
		bytes_reading = read(fd, buffer, BUFFER_SIZE);
		if (bytes_reading == -1)
			return ((free(cup[fd])), (free(buffer)), NULL);
		buffer[bytes_reading] = '\0';
		cup[fd] = ft_strjoin(cup[fd], buffer);
	}
	output = ft_strdup_until_newline(cup[fd]);
	cup[fd] = get_therest(cup[fd]);
	if (output[bytes_reading] == '\0' && bytes_reading == 0)
		return ((free(output)), (free(cup[fd])), (free(buffer)), NULL);
